	...
	public static void main(String[] args) {
		...
		while(!isTrue){
			...
			if(nip.matches("[0-9]*")){
				if(nip.length() >= 12 && nip.length() <= 18){
					...
					System.out.println("Anda belum mengisi NIP / Inputan yang anda masukkan kurang dari 12 angka / lebih dari 18 angka!");
				}
			}else{
		...
		while(!isTrue){
			...
			if(nama.length() > 0 && nama.length() <= 50){
				...
				System.out.println("Anda belum mengisi nama / Inputan yang anda masukkan melebihi 50 huruf!");
			}
		}
		...
		while(!isTrue){
			...
			if(golongan.length() == 2){
				...
				System.out.println("Anda belum mengisi golongan / inputan yang anda masukkan salah!");
			}
		}

		//inputan status pernikahan
		while(!isTrue){
			System.out.print(">> Masukkan status pernikahan anda (berupa nomor) \n   [1.Menikah 2.Belum Menikah 3.Single Parents]\t: ");
			statusNikah = input.nextLine();

			if(statusNikah.length() == 1){
				if (statusNikah.matches("[1-3]*")) {	
					break;
				}
				else{
					System.out.println("Inputan yang anda masukkan salah!");
				...
				System.out.println("Anda belum mengisi status pernikahan / Inputan yang anda masukkan salah!");
			}
		}
		...
		while(!isTrue){
			System.out.print(">> Masukkan jenis PNS anda (berupa nomor)\n   [1.Pejabat 2.Pelayan Masyarakat 3.Lainnya]\t: ");
			jenisPNS = input.nextLine();

			if(jenisPNS.length() == 1){
				...
				System.out.println("Anda belum mengisi jenis PNS / Inputan yang anda masukkan salah!");
			}
		}
		...
		while(!isTrue){
			...
			if(adaTunjanganLain.length() == 1){
				if(adaTunjanganLain.equalsIgnoreCase("y")){
					...
					while(!isTrue){
						...
						if(namaTunjanganLain.length() > 0){
							...
							System.out.println("Anda belum memasukkan nama tunjangan!");
						}
					}
					...
					while(!isTrue){
						...
						if(tunjanganLain.length() > 0){
							...
							System.out.println("Anda belum memasukkan besar tunjangan!");
						}
					}
					break;				
				}
				...
				System.out.println("Anda belum memasukkan inputan / Inputan yang anda masukkan salah!");
			}
		}
