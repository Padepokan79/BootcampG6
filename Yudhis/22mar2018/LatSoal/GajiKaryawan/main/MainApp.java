...
public class MainApp {
	...
	public static void inputData(Karyawan karyawan, int menu) {		
		...
		System.out.print("Lama Kerja (dalam tahun) : "); karyawan.setLamaKerja(Integer.valueOf(validasiAngka("lamaKerja")));
		karyawan.setPenempatanKerja(mapPenempatanKerja.get(inputPenempatanKerja()));
		System.out.print("Lembur (dalam jam) : "); karyawan.setBanyakLembur(Integer.valueOf(validasiAngka("lembur")));
		karyawan.setStatusNikah(inputStatusMenikah());
		
		if (menu == 1) {
			System.out.print("Bugfixing (banyaknya) : "); karyawan.hitungBonus(Integer.valueOf(validasiAngka("bugFixing")));
			// Overload constructor
			programmer = new Programmer(karyawan);
		} else if (menu == 3) {
			System.out.print("Testcase (banyaknya) : "); karyawan.hitungBonus(Integer.valueOf(validasiAngka("testCase")));
			// Overload constructor
